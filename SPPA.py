...
generations = 500  # 遗传算法迭代代数
queue_penalty_weight = 1  # 排队惩罚权重因子
elite_size = 5  # 精英保留的个体数
...
def calculate_adaptive_rates(individual_fitness, avg_fitness, max_fitness, min_fitness):
    ...
    if individual_fitness < avg_fitness:
        # 好个体：保持高交叉率，降低变异率
        crossover_rate = CROSSOVER_MIN + (CROSSOVER_MAX - CROSSOVER_MIN) * normalized_fitness
        # 变异率随适应度增加线性减小
...
def main():
    ...
    # 输出每个采集节点的最优路径及适应度
    for adc, best_individual in global_best_paths.items():
        print(f"采集节点 {adc} 的最优路径: {best_individual.path}")
